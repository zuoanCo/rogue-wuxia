...
export class Game {
    ...
    triggerLevelUp() {
        ...
        options.forEach(opt => {
            ...
            div.innerHTML = `
                <div class="upgrade-title">${opt.name} <span class="upgrade-type">${opt.type}</span></div>
    ...
    triggerChestReward() {
        ...
        options.forEach(opt => {
            const div = document.createElement('div');
            div.className = 'upgrade-card boss-reward'; // Add boss-reward class for styling if needed
            div.style.border = '2px solid #f1c40f'; // Gold border
            div.innerHTML = `
                <div class="upgrade-title" style="color:#f1c40f">${opt.name} <span class="upgrade-type">${opt.type}</span></div>
                <div class="upgrade-desc">${opt.description}</div>
            `;
            div.onclick = () => {
                this.applyUpgrade(opt);
    ...
    getUpgradeOptions(count = 3) {
        ...
        const upgrades = [
            ...
            { id: 'maxhp', name: '易筋经', type: '内功', description: '气血上限提升 20，配合降龙十八掌可进化', apply: (g) => { g.player.maxHp += 20; g.player.hp += 20; g.player.passives.muscleTendon++; } },
            { id: 'multishot', name: '六脉神剑·精进', type: '剑法', description: '增加一道六脉剑气', apply: (g) => {} },
            { id: 'new_wand', name: '六脉神剑', type: '绝学', description: '习得六脉神剑，发射无形剑气', apply: (g) => {} },
            { id: 'aura', name: '九阳神功', type: '绝学', description: '护体真气对周围敌人造成伤害', apply: (g) => {} },
            { id: 'new_dagger', name: '金蛇剑法', type: '奇门', description: '习得金蛇剑法，发射穿透飞剑', apply: (g) => {} },
            { id: 'new_taiji', name: '太极剑法', type: '绝学', description: '习得太极剑法，召唤阴阳剑气护体', apply: (g) => {} },
            { id: 'new_dugu', name: '独孤九剑', type: '绝学', description: '习得独孤九剑，发射追踪剑气', apply: (g) => {} },
            { id: 'new_palm', name: '降龙十八掌', type: '绝学', description: '习得降龙十八掌，发射龙形掌力', apply: (g) => {} },
            { id: 'new_stone', name: '弹指神通', type: '奇门', description: '习得弹指神通，发射弹射石子', apply: (g) => {} },
            { id: 'new_staff', name: '打狗棒法', type: '绝学', description: '习得打狗棒法，召唤打狗棒护身', apply: (g) => {} },
            { id: 'new_roar', name: '狮子吼', type: '绝学', description: '习得狮子吼，发出震慑波', apply: (g) => {} },
            { id: 'new_claw', name: '九阴白骨爪', type: '绝学', description: '习得九阴白骨爪，抓取敌人要害', apply: (g) => {} },
            { id: 'new_seven_star', name: '七星剑', type: '绝学', description: '习得七星剑，布下北斗七星阵', apply: (g) => {} },
            { id: 'new_needle', name: '冰魄银针', type: '暗器', description: '习得冰魄银针，发射带毒银针', apply: (g) => {} },
            { id: 'new_guqin', name: '七弦琴', type: '音律', description: '习得七弦琴，音波攻击穿透敌人', apply: (g) => {} },
            { id: 'new_flying_dagger', name: '小李飞刀', type: '暗器', description: '习得小李飞刀，例不虚发', apply: (g) => {} },
            { id: 'new_huashan', name: '华山剑法', type: '剑法', description: '习得华山剑法，君子之剑', apply: (g) => {} },
            { id: 'passive_snake_tech', name: '金蛇剑法', type: '心法', description: '强化金蛇剑气，满级可化为巨蟒', apply: (g) => { g.player.passives.snakeTech++; } },
            { id: 'passive_taiji_mantra', name: '太极心法', type: '心法', description: '强化太极剑法，满级可化为八卦阵', apply: (g) => { g.player.passives.taijiMantra++; } },
            { id: 'passive_internal_force', name: '北冥神功', type: '心法', description: '增加内力，强化六脉神剑', apply: (g) => { g.player.passives.internalForce++; } },
            { id: 'passive_sword_intent', name: '独孤剑意', type: '心法', description: '领悟剑意，强化独孤九剑', apply: (g) => { g.player.passives.swordIntent++; } },
            { id: 'passive_yang_energy', name: '纯阳无极', type: '心法', description: '修炼纯阳内力，强化九阳神功', apply: (g) => { g.player.passives.yangEnergy++; } },
            { id: 'passive_muscle_tendon', name: '易筋经', type: '心法', description: '易筋洗髓，强化降龙十八掌', apply: (g) => { g.player.passives.muscleTendon++; } },
            { id: 'passive_qimen', name: '奇门遁甲', type: '心法', description: '精通奇门，强化弹指神通', apply: (g) => { g.player.passives.qimen++; } },
            { id: 'passive_beggar_heart', name: '叫花心法', type: '心法', description: '强化打狗棒法，满级可领悟天下无狗', apply: (g) => { g.player.passives.beggarHeart++; } },
            { id: 'passive_zen', name: '坐禅', type: '心法', description: '强化狮子吼，满级可领悟佛门金刚吼', apply: (g) => { g.player.passives.zen++; } },
            { id: 'passive_nine_yin', name: '九阴真经', type: '心法', description: '强化九阴白骨爪，满级可领悟九阴极意', apply: (g) => { g.player.passives.nineYin++; } },
            { id: 'passive_quanzhen', name: '全真剑法', type: '心法', description: '强化七星剑，满级可领悟天罡北斗阵', apply: (g) => { g.player.passives.quanzhen++; } },
            { id: 'passive_poison', name: '五毒秘传', type: '心法', description: '强化冰魄银针，满级可领悟漫天花雨', apply: (g) => { g.player.passives.poison++; } },
            { id: 'passive_melody', name: '逍遥琴谱', type: '心法', description: '强化七弦琴，满级可领悟天龙八音', apply: (g) => { g.player.passives.melody++; } },
            { id: 'passive_truth', name: '天眼通', type: '心法', description: '强化小李飞刀，提高暴击', apply: (g) => { g.player.passives.truth++; } },
            { id: 'passive_ghost', name: '鬼影迷踪', type: '轻功', description: '强化华山剑法，提升速度', apply: (g) => { g.player.passives.ghost++; } },
            { id: 'passive_lifesteal', name: '吸星大法', type: '绝学', description: '攻击附带吸血效果 (每次命中恢复 1 点生命)', apply: (g) => { g.player.passives.lifesteal++; } }
        ];
